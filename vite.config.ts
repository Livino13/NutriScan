    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          ...
          description: 'Scan your food, track calories and macros, build healthier habits.',
          theme_color: '#F0FDF8',
          background_color: '#F0FDF8',
          display: 'standalone',
          start_url: '/',
          icons: [
          ...
          runtimeCaching: [
            {
              ...
              handler: 'CacheFirst',
              options: { cacheName: 'google-fonts-css', expiration: { maxEntries: 10, maxAgeSeconds: 60 * 60 * 24 * 30 } },
            },
            {
              urlPattern: /^https:\/\/fonts\.gstatic\.com\/.*/i,
              handler: 'CacheFirst',
              options: { cacheName: 'google-fonts-files', expiration: { maxEntries: 10, maxAgeSeconds: 60 * 60 * 24 * 30 } },
            },
          ],
        },
      }),
...
/**
 * Dev-only: serve the Vercel-style `api/` functions under `vite dev` so food
 * scan works locally. Production (Vercel) serves `api/` natively — this
 * plugin never runs there (`apply: 'serve'`).
 */
function devApiPlugin(): Plugin {
  return {
    ...
    apply: 'serve',
    configureServer(server) {
      // Make `.env` values (e.g. GEMINI_API_KEY) visible to API handlers in dev.
      ...
      server.middlewares.use('/api/health', (req, res) => {
        void serveDevApi(server, req, res, '/api/health.ts')
      })
      server.middlewares.use('/api/analyze-food', (req, res) => {
        void serveDevApi(server, req, res, '/api/analyze-food.ts')
      })
    },
  }
}

function sendDevJson(res: Record<string, any>, obj: unknown, code?: number) {
  if (code !== undefined) res.statusCode = code
  res.setHeader('Content-Type', 'application/json')
  res.end(JSON.stringify(obj))
}

function readDevJsonBody(req: Record<string, any>): Promise<unknown> {
...
async function serveDevApi(server: Record<string, any>, req: Record<string, any>, res: Record<string, any>, id: string) {
  try {
    const mod = await server.ssrLoadModule(id)
    const handler = mod.default
    if (typeof handler !== 'function') throw new Error(`No default handler in ${id}`)
    if (req.method === 'POST' && String(req.headers?.['content-type'] ?? '').includes('application/json')) {
      try {
        req.body = await readDevJsonBody(req)
      } catch {
        sendDevJson(res, { error: 'Request body too large' }, 413)
        return
      }
    } else {
      req.body = {}
    }
    res.status = (code: number) => ({ json: (obj: unknown) => sendDevJson(res, obj, code) })
    await handler(req, res)
  } catch (err) {
    console.error(`[dev-api] ${id} failed:`, err)
    if (!res.headersSent) sendDevJson(res, { error: 'Dev API handler failed' }, 500)
  }
}
/**
